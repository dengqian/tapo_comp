...
cut -d ',' -f 4 $1/ack_loss.txt | cut -d '=' -f 2 | awk -v sum1=$sum1 '{sum += $1} END {print sum,"ack_loss:", sum/sum1}'
...
cut -d ',' -f 4 $1/recovery_loss.txt | cut -d '=' -f 2 | awk -v sum1=$sum1 '{sum += $1} END {print sum,"recovery_loss:", sum/sum1}'
