...
static inline int hash(struct tcp_key *key)	// make the key distrabute evenly, =o= do know the purpose
{
	assert(BITS < 24);	//declare in hash_table.h, inited to 20

	int val = 0;
	int shift = 3;
	int size = sizeof(struct tcp_key)*8;	//sizeof tcp_key is 12 Bytes
...
struct tcp_state *find_ts_entry(struct hash_table_entry **hash_table, struct tcp_key *key)
{
	...
	while (entry) {
		if (memcmp(&entry->ts->key, key, sizeof(struct tcp_key)) == 0)	// int memcmp(const void *s1, const void *s2, size_t n);
...
int insert_ts_entry(struct hash_table_entry **hash_table, struct tcp_state *ts) //insert in front of node with key value ts->key
{
	int hv = hash(&ts->key);
